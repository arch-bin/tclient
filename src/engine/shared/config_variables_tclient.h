...
MACRO_CONFIG_INT(TcAntiVoid, tc_anti_void, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Avoid: physics-predicted input blocking. Simulates ahead with your current input and overrides hook/direction only when the prediction says you die otherwise")
MACRO_CONFIG_INT(TcAvoidCheckTicks, tc_avoid_check_ticks, 40, 5, 100, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Avoid: prediction window in ticks (50 = 1s). The simulation runs this far ahead to see a death coming; higher = spots danger earlier but more CPU")
...
MACRO_CONFIG_INT(TcAntiVoidDeath, tc_anti_void_death, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Avoid treats death (kill) tiles as deadly")
...
MACRO_CONFIG_INT(TcAntiVoidLiveFreeze, tc_anti_void_live_freeze, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Avoid treats live freeze tiles as deadly")
...
MACRO_CONFIG_INT(TcAvoidThreadFreeze, tc_avoid_thread_freeze, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Avoid: ONLY for maps where freeze is harmless and you dive through it on purpose. ON = do NOT release your hook just because it pulls you through freeze; only release for a REAL kill/deep/edge. WARNING: on a map whose 'void' IS freeze this disables your freeze protection — the hook will happily pull you up into freeze. OFF (default) = freeze counts, so the hook is released before it drags you into it")
MACRO_CONFIG_INT(TcAntiVoidShow, tc_anti_void_show, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Avoid: draw the predicted path and the dangerous tiles around you")
